...
test.describe('F02 NPC对话修习 - E2E 测试', () => {
    ...
    test('F02-AC-004: aside区域应包含搜索卡牌输入框', async ({ page }) => {
        await navigateToDialogue(page)

        await expect(page.getByLabel('搜索卡牌')).toBeVisible({ timeout: 15000 })

        const searchInput = page.getByLabel('搜索卡牌')
        await expect(searchInput).toHaveAttribute('placeholder', /搜索或新建卡牌/)
    })
